...
mturk_url = "https://kbpo.stanford.edu/tasks/do"
...
def create_batch(db, mturk_connection, batch_id, range_type=None, range_begin=None, range_end=None):
    ...
    batch_type, evaluation_batch_params = next(db.select(
        "SELECT batch_type, params from evaluation_batch WHERE id = %(batch_id)s", batch_id=batch_id))
    if range_type is None:
        assert range_begin is None and range_end is None, "Specify range type"
    elif range_type is 'percentage':
    ...
    questions = db.select(
        ...
        limit=range_end-range_begin
        )

    #TODO: Have a meaningful autogenerated description
    with db.CONN:
        with db.CONN.cursor() as cur:
            db.execute_without_commit(cur, """INSERT INTO mturk_batch (params, description)
                                      VALUES (%(mturk_params)s, %(description)s) RETURNING id""",
            ...
            for question in questions:
                ...
                db.execute_without_commit(cur,
                    """INSERT INTO mturk_hit (id, batch_id, question_batch_id, question_id, type_id, price, units)
                       ...
                       %(question_id)s, %(hit_type_id)s, %(price)s, %(units)s)""",
                    hit_id=hit_id,
