...
def parse_selective_relations_response(question, response):
    ...
    mentions = []
    canonical_mentions = [
        CanonicalMentionInstance(subject_id,  subject_canonical_id, 1.0 if response["subject"]["entity"]["canonicalCorrect"] == "Yes" else 0.0),
        CanonicalMentionInstance(object_id,  object_canonical_id, 1.0 if response["object"]["entity"]["canonicalCorrect"] == "Yes" else 0.0),
        ]
    links = [
        LinkInstance(subject_id, response["subject"]["entity"]["link"], 1.0 if response["subject"]["entity"]["linkCorrect"] == "Yes" else 0.0),
        LinkInstance(object_id, response["object"]["entity"]["link"], 1.0 if response["object"]["entity"]["linkCorrect"] == "Yes" else 0.0),
        ]
    relations = [RelationInstance(subject_id, object_id, response["relation"], 1.0)]
    return mentions, canonical_mentions, links, relations
...
def test_parse_exhaustive_relations_response():
    ...
    # - the mention could be wrong. (TODO: how are we handling this?)
    # - the linking could be wrong.
    # - the canonical could be wrong. (TODO: we need to factor out our representation to handle this correctly)
    # - the relation could be wrong.
