...
In knowledge base population,
  each relation is a triple (\textsc{subject}, \textsc{predicate}, \textsc{object}) where \textsc{subject} and \textsc{object} are some globally unique entity identifiers (e.g.\, Wikipedia page titles), \textsc{predicate} is a relation \pl{can't use relation here!} belonging to a specified schema.\footnote{%
...
A KBP system returns an output in the form of \textit{relation instances} (\textsc{subject}, \textsc{predicate}, \textsc{object}, \textsc{provenance}), where \textsc{provenance} is a description of where exactly in the document corpus the subject, object and relation \pl{you mean predicate?} were found. 
...
Separately, a team of annotators identify relation instances for the same evaluation entities $E$ by manually searching the document corpus within a time budget \pl{these instances are not labeled?}.
...
A system is evaluated on the precision of its predicted relation instances for the evaluation entities $E$ and on the recall of the \textit{relations} \pl{not relation instances? why asymmetry?} it predicts for the same entities (see \reffig{pooling} for a worked example).
