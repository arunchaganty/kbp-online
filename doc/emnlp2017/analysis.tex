...
The bias affects precision and recall almost equally: 
  ...
  At the same time, the median difference between two adjacent submissions is only 0.3\%\fone{} with largest difference being 1.5\% \fone{} (between the 26th and 27th ranked submissions) \ap{is lesser than the pooling bias with \anydoc{} heuristic}.

  We also considered an alternate scoring model of ignoring relational tuples outside the evaluation set during scoring and find that under this model \fone{} scores remain biased by 14.99\% and 0.73\% points on average, with and without the \anydoc{} heuristic respectively. \ap{Methodology is unclear. We should state what precision and recall mean in a sentence or what the TP/FP values etc are now counting}
