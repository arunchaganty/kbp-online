...
The original evaluation dataset contains labels for the relational tuples from all the participating systems, so we can remove any tuples that are unique to a particular system to simulate what the evaluation dataset would be if that system did not participate in the pooling process.
...
\paragraph{Results.}
\figureref{pooling-bias} shows the results of the leave-out-unique experiment on the TAC-KBP 2015 competition on the \fone{} metric with and without the \anydoc{} heuristic.
...
The bias affects precision and recall almost equally: 
  ...
  At the same time, the median difference between two adjacent submissions is only 0.3\%\fone{} with largest difference being 1.5\% \fone{} (between the 26th and 27th ranked submissions), less than the pooling bias with \anydoc{} heuristic.
