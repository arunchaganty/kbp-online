...
% Primer
While it is apparent from the example in \figureref{pooling} that the pooling evaluation methodology could introduce bias, the argument for its use has been that this bias would be insignificant if enough systems were pooled.
...
\begin{figure*}[t]
  ...
  \begin{subfigure}{\columnwidth}
      \includegraphics[width=\columnwidth, trim={0 8cm 0 0},clip]{figures/pooling_bias/pooling_bias_closed-world}
    \centering
    \small{\begin{tabular} {r r r } \toprule
       Median Precision & Median Recall & Median \fone{} \\ \midrule 
 17.93\% &  17.00\% & 15.51\% \\ \bottomrule
   \end{tabular}}
  \caption{\label{fig:exact-prov-pooling}
Pooling bias for macro \fone{} scores with exact provenance match.
   }

  \end{subfigure}
  \begin{subfigure}{\columnwidth}
  \includegraphics[width=\columnwidth, trim={0 8cm 0 0},clip]{figures/pooling_bias/pooling_bias_anydoc}
    \centering
    \small{
   \begin{tabular} {r r r } \toprule
      Median Precision & Median Recall & Median \fone{} \\ \midrule 
 2.34\% &  1.93\% & 2.05\% \\ \bottomrule
   \end{tabular}}
  \caption{\label{fig:anydoc-pooling}Pooling bias for macro \fone{} scores with the \anydoc{} heuristic.
  }
  \end{subfigure}
...
To measure pooling bias, we'll need to recreate the conditions shown in \figureref{pooling}: we must be able to measure evaluation metrics for a system before and after its inclusion to the pooled dataset. 
...
\paragraph{Results.}
\figureref{pooling-bias} shows the results of the leave-out-unique experiment on the TAC-KBP 2015 competition on the \fone{} metric with exact provenance match (\figureref{exact-prov-pooling}) and with the \anydoc{} heuristic (\figureref{anydoc-pooling}).
...
We observe that even with lenient \anydoc{} heuristic the median bias is still a significant 2.05\% \fone{}. In comparison the median difference between two adjacent submissions is only 0.3\%\fone{} with largest difference being 1.5\% \fone{} (between the 26th and 27th ranked submissions), less than the pooling bias with \anydoc{} heuristic.

  We also considered an alternate scoring model where relational tuples outside the evaluation set were of ignored instead of marked wrong and find that under this model \fone{} scores remain biased by 14.99\% and 0.73\% points on average, with and without the \anydoc{} heuristic respectively.
...
% % We can't do that, but we can compare how teams that participated in the competition might have fared had they not been part of the evaluation. 
