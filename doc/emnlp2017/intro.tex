...
In particular, knowledge base population seeks to automatically construct a knowledge base consisting of relations between entities from a document corpus. For example, ``\entity{Carrie Fisher}'s \relation{mother} is \entity{Debbie Reynolds}''.
...
As a result, a pooling-based methodology is used in practice to construct datasets, similar to the Cranfield evaluation methodology in information retrieval.
For instance, at the annual NIST TAC-KBP evaluation, relations predicted by participating systems are pooled together, labeled and released as a dataset for researchers to develop and evaluate their systems.
However during development, if a novel system predicts a new relation, not predicted by any of the systems in the pool, it is considered to be wrong by default. 


% Key finding: pooling bias
The key finding of this paper is that the pooling methodology is biased against novel systems during development and penalizes them by 2\% \fone{} on average. 
Novel improvements, which typically increase scores by less than 1\% \fone{} after a pooled evaluation, are likely to be subsumed by the pooling bias during development. 
%This can mask any significant improvements due to the novel system which typically increase scores by less than 1\% \fone{},
...
Worse yet, the bias is stronger for a system which predicts qualitatively different relations that were missing systematically from the pool.
...
%While designers of rule-based systems can still be guided by their intuition, those who rely on empirical tuning and machine learning are likely to be directed towards only predicting what others have predicted and no more.

% anti-solution
Of course, systems participating in the TAC-KBP evaluation do not suffer from the studied bias, but this requires researchers to wait a year to get credible feedback on new ideas.
%, and the competition itself is an elaborate undertaking.
%
...
Instead, we propose a new evaluation methodology we call on-demand evaluation.
...
We \fake{simulate our framework on evaluation data released through TAC-KBP Slot Validation tracks and show that we are able to obtain unbiased estimates while only labeling a fraction of the data}.
We also run a mock evaluation of three distinct systems on the 2016 TAC-KBP corpus and find that we are able to estimate precision and recall within \fake{2\% \fone{} (half as much as the official scores)} and within a budget of \$3000 (a fraction of the cost of the official evaluation). 
We hope that the availability of immediate, unbiased evaluation
