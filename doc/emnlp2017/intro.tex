\begin{figure}[t]
  ...
  In pooled evaluation, an evaluation dataset is constructed by labeling relation instances collected from the pooled systems (A and B) and those identified by a team of human annotators (Annotators).
 % Arrows indicate which systems extract which relation instances.
  However, when a new system (C) is evaluated on this dataset, some of its predictions are previously unseen (e.g., $t_6$) and can not be fairly evaluated.
...
We believe the existence of this service will speed the pace of progress in developing KBP systems.
