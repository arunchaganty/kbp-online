...
%(e.g.\, by exhaustively annotating a document \pl{this might be confusing since
%annotating a document does more than produce just one sample - actually not
%sure how it provides a random draw from $\sY$})
%at a certain (monetary) cost.
...
\subsection{Independent estimators}
...
Then, the independent estimators for precision and recall are:
\begin{align}
  \pih_i^{\text{(indep)}} &= \frac1{n_i} \sum_{x \in \Xh_i} f(x) & \rhoh_i^{\text{(indep)}} &= \frac1{n_0} \sum_{x \in \Yh_0} g_i(x).
\end{align}

\subsection{Joint estimators}
The independent estimators are unbiased but have wastefully large variance
