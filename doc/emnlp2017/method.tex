...
The missing factor must compensate for how much of $\sY$ the pool $Y$ actually covers, i.e.\ the pool recall: $\theta \eqdef \E_{x \sim \sY}[g(x)]$ where $g(x) \eqdef \I[x \in X]$.
...
We can measure how well the current evaluation set is able to estimate $\pih_m^{(j)}$ and $\rhoh_m^{(j)}$ by using upper bounds on these of estimators.\footnote{Details can be found in \refapp{sampling} of the supplementary material.}
