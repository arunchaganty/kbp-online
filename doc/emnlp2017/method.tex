...
We assume that $p_i(x)$ is known, e.g.\, the uniform distribution over $X_i$
  and that we know $p'(x)$ up to normalization constant, i.e.\ we know $p'(x) \propto u(x)$ \pl{how about $p_0$ rather than $p$' everywhere? more consistent with other notation}.
Finally, we assume that we are able to draw samples uniformly from $\sY$.

In on-demand evaluation, we are allowed to ask if $x \in \sY$ (e.g.\, by labeling an instance) or for fresh samples from $\sY$ (e.g.\, by exhaustively annotating a document \pl{this might be confusing since annotating a document does more than produce just one sample - actually not sure how it provides a random draw from $\sY$}) at a certain (monetary) cost.
Typically, checking if $x \in \sY$ is significantly cheaper than asking for samples from $\sY$.
\pl{abrupt transition}
...
\end{align}
\pl{need to divide by the number of samples, no?}
\pl{subscript $(s)$ stands for simple?  I'd do $(\text{s})$ to make it not look like a variable}

\subsection{Joint estimator}
The simple estimator leaves much to be desired: in particular, because it does not use samples collected from other systems, it will have a larger variance and requires us to spend money to collect data for every new system submitted.
We'll address these problems by answering the following three key questions:
\begin{enumerate}
  \item How can we use all the samples $\Xh_1, \ldots \Xh_m$ when estimating precision $\pi_i$?
  \item How can we combine all the samples $\Xh_1, \ldots, \Xh_m$ with $\Yh_0$ when estimating recall $\rho_i$?
  \item Finally, to form $\Xh_m$, how many samples should we draw from $X_m$, given existing samples $\Yh_0$ and $\Xh_1, \ldots, \Xh_{m-1}$?
\end{enumerate}
...
\end{align}
\pl{I don't understand this equation at all; for example, LHS has $j$ in the superscript, but RHS sums over $j$}
...
The variance depends on $f(x)$, but the general intuition is that $\pih_i^{(j)}$ will have high variance if $q_i(x) \ll p_i(x)$ \pl{what does this mean? there exists an $x$ such this is true? you're comparing functions}.
...
Finally, a desired property for on-demand evaluation is to annotate \pl{use consistent terminology: label} new data \pl{instances} only when the current evaluation data \pl{I don't think this was ever defined formally - set of predicted instances and their labels?} is unable to do the job, e.g.\ when a new set of predictions $X_m$ contains many instances not covered by other systems.
...
\paragraph{Summary.}
\pl{this would be better to have first as an outline before all the formulas!}
