...
We would like to evaluate the expectation of a metric (e.g.\ precision) $f$: $\E_{p_i(x)}[f(x)]$ using samples from each $X_i$. We further assume that $f(x)$ is bounded between $0$ and $1$.
Additionally, we would like to evaluate the expectation conditioned on an event $g(x) = 1$ (recall): $\E_{p_i(x)}[f(x) \given g(x)]$.
...
Q. Can I quantify the mass to place on a particular thing based on set overlap? That's one obvious way of doing it with set sizes.

\paragraph{Estimating recall}
...
% NOTE: it doesn't make any sense to have anything but uniform distribution over true relation instaces -- how we ould define a relation instance version of this? We can only stratify our metrics by relation or entity.

% CHALLENGE is using the pooled samples in estimating.
...
\begin{algorithm}
\begin{algorithmic}
  \REQUIRE{Sets $X_1, \ldots, X_m$ and distributions $p_1, \ldots, p_m$}
  \ENSURE{A set of sampled elements, $Y$.}
  \STATE{$Y \gets \emptyset$.}
  \FOR{$i=1$ \TO{} $i=m$}
  \STATE{$Y_i = $ sample without replacement from $X_i \setminus Y$.}
  \STATE{$Y \gets$ $Y \union Y_i$.}
\ENDFOR{}
\end{algorithmic}
\end{algorithm}
